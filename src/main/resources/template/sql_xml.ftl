...
<mapper namespace="${daoPackage}.${table.className}Mapper">
    ...
    <insert id="saveSelective" <#if pk.extra == 'auto_increment'>useGeneratedKeys="true" keyProperty="${pk.attrname}"</#if>
                parameterType="${doPackage}.${table.className}DO">
            INSERT INTO ${table.tableSchema}.${table.tableName}(
            <trim suffix="" suffixOverrides=",">
                <#list columns as c>
                <#if c.extra != 'auto_increment' && c.extra != "on update CURRENT_TIMESTAMP" && c.columnName != "created_at">
